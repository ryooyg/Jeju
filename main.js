...
jejuolle_check.addEventListener('change', function () {
    if (this.checked) {
        $.ajax({
            url: './geojson/jeju_olle_trail.geojson',
            dataType: 'json',
...
jejuhalla_check.addEventListener('change', function () {
    if (this.checked) {
        $.ajax({
            url: './geojson/jeju_halla_trail.geojson',
            dataType: 'json',
...
jejuzpt_check.addEventListener('change', function () {
    if (this.checked) {
        $.ajax({
            url: './gpx/ZEROPOINTTRAIL THIRD STEP 1DAY 31KM.gpx',
            dataType: 'xml',
            success: startzptDataLayer            
        });

    } else {          
        map.data.removeGpx(jejuzpt);        
    }
});

function startzptDataLayer(XmlDoc) {    
